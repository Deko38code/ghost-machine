...
//   vidsrcme.ru → redirector
//   vidsrc.dev → self-hosted JS player
...
const STREAM_SOURCES = {
  ...
  vidsrcdev:   { name: '🔥 VidSrc Dev',   movie: ids => `https://vidsrc.dev/embed/movie/${ids.imdbId || ids.tmdbId}`, tv: (ids,s,e) => `https://vidsrc.dev/embed/tv/${ids.imdbId || ids.tmdbId}/${s}/${e}` },
  vidsrcme:    { name: '📡 VidSrc Me',   movie: ids => `https://vidsrcme.ru/embed/movie/${ids.imdbId || ids.tmdbId}`, tv: (ids,s,e) => `https://vidsrcme.ru/embed/tv/${ids.imdbId || ids.tmdbId}/${s}/${e}` },
  ...
  portal:      { name: '💀 Portal',       movie: null, tv: null, portal: true },  // Fallback: uses portal hits
};

// Source priority order — only LIVE sources (tested May 2026)
// Dead sources removed: vidsrc.xyz, vidsrc.icu, vidsrc.cc, autoembed, blackvid, moviesapi.club, vidsrc.pro(embed.su DNS fail)
const SOURCE_ORDER = ['vidsrc2','vidsrcpm','vidsrcdev','vidsrcme','multiembed','embed2','embed2skin','vidsrcpro','goojara','playmogo','lookmovie','portal'];
...
const CoverArtCache = {
  ...
  async prewarm() {
    try {
      // Load ALL entries from server bank
      const res = await fetch('/api/cover-bank?all=1');
      if (res.ok) {
        const data = await res.json();
        if (data.entries) {
          const bank = this._load();
...
class CinemetaApi {
  ...
  async _get(path) {
    ...
    const res = await fetch(`/api/cinemeta${path}`);
  ...
  async tvDetails(imdbId) {
    const data = await this._get(`/meta/tv/${imdbId}.json`);
    return data?.meta || null;
  }
}
